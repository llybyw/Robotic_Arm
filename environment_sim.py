...
class Environment:
    def __init__(self, gui=True, time_step=1 / 240):
        ...
        self.oracle_cams1 = cameras.Oracle1.CONFIG
    ...
    def setup_gripper(self):
        ...
        self.gripper_angle_close = 0.4
        # self.gripper_angle_close_threshold = 0.77
        self.gripper_angle_close_threshold = 0.2
    ...
    def render_camera(self, config):
        ...
        width, height, color, depth, segm = pb.getCameraImage(
        ...
        color = color[:, :, :3]  # remove alpha channel 彩图
        # imageshow = cv2.cvtColor(color, cv2.COLOR_RGB2BGR)#彩图可视化
        # cv2.imshow("1", imageshow)
        # cv2.waitKey(0)
        ...
        # plt.imshow(depth, cmap='hot')#深度图可视化
        # plt.colorbar()
        # plt.title('111')
        # plt.show()
        if config["noise"]:
    ...
    def add_objects(self, num_obj, workspace_limits):
        ...
        # print(len(mesh_list))
        # yellow_bowl = "assets/unseen_objects/yellow_bowl.urdf"
    ...
    def add_object_push_from_file1(self, file_name, switch=None):
        success = True
        # Read data
        with open(file_name, "r") as preset_file:
            file_content = preset_file.readlines()
    ...
    def add_object_push_from_file2(self, file_name, switch=None, workspace_limits=None):
        success = True
        # Read data
        with open(file_name, "r") as preset_file:
            file_content = preset_file.readlines()
            ...
            for object_idx in range(num_obj):
                ...
                workspace_limits_list = [
                    [[0.276, 0.724], [-0.224, -0.04], [-0.0001, 0.4]],  # 第一块区域
                    [[0.276, 0.724], [0.04, 0.224], [-0.0001, 0.4]],  # 第二块区域
                    [[0.276, 0.42], [-0.224, 0.224], [-0.0001, 0.4]],  # 第三块区域
                    [[0.58, 0.724], [-0.224, 0.224], [-0.0001, 0.4]],  # 第四块区域
                ]

                chosen_workspace = workspace_limits_list[np.random.randint(len(workspace_limits_list))]

                # 随机生成位置（在给定的 workspace_limits 内）
                drop_x = (
                        (workspace_limits[0][1] - workspace_limits[0][0] - 0.1) * np.random.random_sample()
                        + workspace_limits[0][0]
                )
                drop_y = (
                        (workspace_limits[1][1] - workspace_limits[1][0] - 0.1) * np.random.random_sample()
                        + workspace_limits[1][0]
                )
                drop_z = workspace_limits[2][0]  # 默认高度（可以固定为某个值）
                obj_positions.append([drop_x, drop_y, drop_z])
    ...
    def add_object_push_from_file3(self, file_name, switch=None, workspace_limits=None):
        success = True
        # Read data
        with open(file_name, "r") as preset_file:
            file_content = preset_file.readlines()
            ...
            for object_idx in range(num_obj):
                ...
                workspace_limits_list = [
                    [[0.276, 0.724], [-0.224, -0.04], [-0.0001, 0.4]],  # 第一块区域
                    [[0.276, 0.5], [0.04, 0.224], [-0.0001, 0.4]],  # 第二块区域
                    [[0.276, 0.42], [-0.224, 0.224], [-0.0001, 0.4]],  # 第三块区域
                    [[0.58, 0.724], [-0.224, 0.184], [-0.0001, 0.4]],  # 第四块区域
                ]
                ...
                # 随机生成位置（在给定的 workspace_limits 内）
                drop_x = (
                        (chosen_workspace[0][1] - chosen_workspace[0][0] - 0.05) * np.random.random_sample()
                        + chosen_workspace[0][0]
                )
                drop_y = (
                        (chosen_workspace[1][1] - chosen_workspace[1][0] - 0.05) * np.random.random_sample()
                        + chosen_workspace[1][0]
                )
                drop_z = chosen_workspace[2][0]  # 默认高度（可以固定为某个值）
                obj_positions.append([drop_x, drop_y, drop_z])
    ...
    def add_object_push_from_file4(self, file_name, switch=None, workspace_limits=None):
        success = True
        # Read data
        with open(file_name, "r") as preset_file:
            file_content = preset_file.readlines()
            ...
            for object_idx in range(num_obj):
                ...
                workspace_limits_list = [
                    [[0.58, 0.724], [-0.224, 0.224], [-0.0001, 0.4]],  # 第一块区域
                    [[0.276,  0.42], [-0.224, 0], [-0.0001, 0.4]],  # 第二块区域
                    [[0.276, 0.42], [0.2, 0.224], [-0.0001, 0.4]],  # 第三块区域
                ]
    ...
    def get_true_object_pose(self, obj_id):
        ...
        return transform, pos, ort

    def get_true_object_poses(self):
        transforms = dict()
        for obj_id in self.obj_ids["rigid"]:
            transform, _, _= self.get_true_object_pose(obj_id)
    ...
    def go_home(self):
        return self.move_joints(self.home_joints, speed=0.0005)

    def move_joints(self, target_joints, speed=0.0005, timeout=5):
    ...
    def straight_move(self, pose0, pose1, rot, speed=0.0005, max_force=300, detect_force=False, is_push=False):
    ...
    def grasp(self, pose, pose2, speed=0.00005):
        ...
        pb.changeDynamics(
            self.ee, self.ee_finger_pad_id, lateralFriction=10, spinningFriction=5
        )
        ...
        pos[2] = max(pos[2] - 0.07, self.bounds[2][0])
        over = np.array((pos[0], pos[1], pos[2] + 0.3))
        ...
        if success:
            ...
            '''
        if success:
            print(f"Moving to pose2: {pose2}")
            # joints = self.solve_ik(pose2)
            # print(f"joints: {joints}")
            # success = self.move_joints(joints)
            success = self.move_ee_pose(pose2, speed)
            if not success:
                print("Failed to move to pose2.")
            time.sleep(1)
    ...
    @property
    def is_grasped_obj(self):
        """
        判断机械夹爪是否夹住物体。
        条件：
        - 当前夹爪角度小于初始角度
        - 机械夹爪没有完全闭合（表明夹住了物体而不是完全闭合状态）

        Returns:
            bool: True 如果夹住物体，False 否则。
        """
        # 获取当前夹爪的角度
        current_angle = pb.getJointState(
            self.ee, self.gripper_main_joint, physicsClientId=self._client_id
        )[0]
        print(f'current_angle', current_angle)
        # 判断夹爪是否夹住物体
        print('gripper_angle_close', self.gripper_angle_close)
        if current_angle < self.gripper_angle_close and self.is_gripper_closed:
            print("Gripper has grasped an object!")
            return True
        else:
            print("Gripper did not grasp any object.")
            return False
    ...
    def acquire_train_image(self, num_obj, workspace_limits):
        """Randomly dropped objects to the workspace"""
        mesh_list = glob.glob("assets/simplified_objects/*.urdf")
        # print(len(mesh_list))
        # yellow_bowl = "assets/unseen_objects/yellow_bowl.urdf"
        obj_mesh_ind = np.random.randint(0, len(mesh_list), size=num_obj-1)
        # get target object
        body_ids = []
        with open("cases/" + "temp.txt", "w") as out_file:
            out_file.write("%s\n" % self.lang_goal)
        ...
        return body_ids, True
    def acquire_train_image1(self, num_obj, workspace_limits,class_id):
        mesh_list = glob.glob("assets/simplified_objects/*.urdf")

        # get target object
        target_mesh_list = []
        target_objs = ["{}".format(str(class_id).zfill(3))]
        ...
        obj_mesh_ind = np.random.randint(0, len(mesh_list), size=num_obj-len(target_objs))
        ...
        # oriten = [0,0,0]
        # stat_id = pb.loadURDF(
        #             yellow_bowl, [0,0,0], pb.getQuaternionFromEuler(oriten)
        #         )
        # body_ids.append(stat_id)
        # self.wait_static()
        self.target_obj_ids = []
